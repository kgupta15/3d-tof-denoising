...
param = np.random.random(18)
img = np.expand_dims(img, axis=0)
img = np.expand_dims(img, axis=1)
out_img = gen(torch.from_numpy(img).type(torch.FloatTensor).to(device), torch.from_numpy(param).type(torch.FloatTensor).to(device), torch.from_numpy(img).type(torch.FloatTensor).to(device))
print(out_img.shape)
